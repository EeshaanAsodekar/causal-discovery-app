...
from data.data_load import load_fi_cmdty_data, get_fi_cmdty_data, calculate_percentage_change
...
def causal_discovery_page() -> None:
    st.subheader("FI & Commodity Causal Discovery Exploration")
    st.write("Currently under development!")
    # """
    # Streamlit page for FI & Commodity Causal Discovery Exploration.
    # Allows users to select specific instruments and date ranges for analysis.
    # The filtered returns data will be used as input for VARLiNGAM.
    # """
    # st.header("FI & Commodity Causal Discovery Exploration")

    # # Load and cache the dataset
    # df = load_fi_cmdty_data()

    # # Get the unique symbols (ticker symbols) from the 'symbol' column
    # unique_symbols = df['ticker_symbol'].unique()

    # # Create a multi-select widget for users to select specific ticker symbols
    # selected_symbols = st.multiselect(
    #     'Select instruments (ticker symbols) for analysis:',
    #     unique_symbols
    # )

    # # Filter the data based on the user's selection
    # if selected_symbols:
    #     filtered_df = get_fi_cmdty_data(df, selected_symbols)
    #     return_df = calculate_percentage_change(filtered_df)
    ...
    #     min_date = pd.to_datetime(return_df['date'].min())
    #     max_date = pd.to_datetime(return_df['date'].max())

    #     # Create date input widget for selecting the date range
    #     start_date = st.date_input('Start date:', min_value=min_date, max_value=max_date, value=min_date)
    #     end_date = st.date_input('End date:', min_value=min_date, max_value=max_date, value=max_date)

    #     # Filter the return DataFrame based on the selected date range
    #     if start_date and end_date:
    #         mask = (return_df['date'] >= pd.to_datetime(start_date)) & (return_df['date'] <= pd.to_datetime(end_date))
    #         filtered_returns_df = return_df[mask]

    #         # Display filtered returns
    #         st.write("Filtered Returns for Selected Date Range")
    #         st.write(filtered_returns_df)

    #         # Now you can pass `filtered_returns_df` as input for VARLiNGAM or any other analysis
    #     else:
    #         st.write("Please select a valid date range.")

    # else:
    #     st.write("Please select at least one instrument for analysis.")
