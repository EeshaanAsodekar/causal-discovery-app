...
def apply_varlingam_TS_per_year(df_dict, lags=1, top_n=10, bootstrap=False, n_sampling=5):
    """
    Apply the VARLiNGAM model to the dataframes in df_dict for each year/period and extract time-varying coefficients.
    ...
    Returns:
        time_series_results (dict): A dictionary storing the time-varying coefficients for analysis.
    """
    time_series_results = {}  # To store time-varying causal coefficients
    ...
    for feature, df_variable in df_dict.items():
        ...
        st.info(f"Applying VARLiNGAM for {feature} across years with {lags} lag(s)...")

        # Split the data by year or any other time period
        df_variable['year'] = df_variable.index.year
        unique_years = df_variable['year'].unique()
        time_series_matrices = []

        # Apply VARLiNGAM for each year
        for year in unique_years:
            df_year = df_variable[df_variable['year'] == year].drop(columns='year')

            # Ensure df_year has at least two rows and two columns
            if df_year.empty or df_year.shape[0] <= lags or df_year.shape[1] < 2:
                st.warning(f"Skipping year {year} for {feature} due to insufficient data.")
                continue
            ...
            if bootstrap:
                result = model.bootstrap(df_year, n_sampling=n_sampling)
                adjacency_matrices = result.adjacency_matrices_
            else:
                model.fit(df_year)
                adjacency_matrices = model.adjacency_matrices_

            # Store the adjacency matrix for the year
            time_series_matrices.append((year, adjacency_matrices[0]))  # Store the first lag only

        # Store the time-varying coefficients for the feature
        time_series_results[feature] = {
            'adjacency_matrix_series': time_series_matrices,  # Store the adjacency matrices over time
            'original_labels': df_variable.columns.to_list()
        }

        # Ask the user to pick two stocks and show coefficient evolution over time
        st.subheader(f"Time Series of Causal Coefficients for {feature}")
        org_labels = df_variable.columns.drop('year').to_list()
        selected_pair = st.multiselect(f"Select two stocks for {feature} to view their causal relationship over time:",
                                       org_labels, default=org_labels[:2])

        if len(selected_pair) == 2:
            # Extract the time-varying coefficients between the two selected stocks
            plot_coefficient_time_series_per_year(time_series_matrices, org_labels, selected_pair)

    return time_series_results

def plot_coefficient_time_series_per_year(time_series_matrices, labels, selected_pair):
    """
    Plot the time series of causal coefficients between two selected stocks over years.

    Parameters:
        time_series_matrices (list): A list of (year, adjacency matrix) tuples from the VARLiNGAM model.
        labels (list): A list of stock ticker labels.
        selected_pair (list): The selected pair of stocks to plot the time-varying coefficients.
    """
    ...
    # Extract the time series of causal coefficients between the two stocks for each year
    years = []
    coefficients_1_to_2 = []
    coefficients_2_to_1 = []

    for year, adj_matrix in time_series_matrices:
        years.append(year)
        coefficients_1_to_2.append(adj_matrix[idx_1, idx_2])
        coefficients_2_to_1.append(adj_matrix[idx_2, idx_1])

    # Plot the time series of causal coefficients
    ...
    plt.title(f'Causal Coefficients Over Time Between {selected_pair[0]} and {selected_pair[1]}')
    plt.legend()
    plt.grid(True)
    st.pyplot(plt)
...
def causal_discovery_page():
    ...
    # Multi-select for users to choose specific tickers
    selected_tickers = st.multiselect("Select Firms (Tickers) for Analysis:", tickers)
    ...
    varlingam_results = apply_varlingam_TS_per_year(df_dict, lags=selected_lags)
